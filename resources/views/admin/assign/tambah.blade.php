		...
		<div class="col-12 grid-margin">
			<div class="card">
				<div class="card-body">
					...
					<div class="row">
						<div class="col-md-12">
							<form action="{{ route('admin.assign.store') }}" method="POST" enctype="multipart/form-data">
								@csrf
								<div class="flex">
									<div class="w-3/4">
										...
										<div class="form-group">
											...
											<select class="form-control" name="kedatangan_id" id="exampleFormControlSelect2">
												@foreach($kedatangan as $ikan)
												<option value="{{ $ikan->id }}">{{ $ikan->code }} - {{ $ikan->fish->name }} - {{ $ikan->grade->name }} - {{ $ikan->size->name }}</option>
												@endforeach
											</select>
										</div>
										<div class="form-group">
											<label for="exampleFormControlSelect2">Pilih Rack</label>
											<select class="form-control" name="rack_id" id="exampleFormControlSelect2">
